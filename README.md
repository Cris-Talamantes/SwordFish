...
3. Vercel reads `vercel.json`: it builds the Vite app from `frontend/` and serves Python from `api/index.py`. Root `requirements.txt` mirrors `backend/requirements.txt` for the serverless runtime.
...
| `VITE_API_URL` | No | Leave **unset** on Vercel so the browser calls `/api` on the same hostname |
